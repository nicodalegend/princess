...
const SITE_CONFIG = {
  ...
  discordUrl: "https://discord.com/invite/ZhsbDQanbA",
  discordLabel: "Join Discord",
  discordDesc: "My community server is live — come hang out!",

  // Paste your Techdom Programs URL here when it's ready
  techdomUrl: "",
  techdomLabel: "View Programs",
  techdomDesc: "My Techdom programs and projects are live!",
};
...
function unlockWhenReady({ url, label, desc, cardId, buttonId, footerId }) {
  ...
  const descEl = card ? card.querySelector(".card__desc") : null;
...
// The Techdom Programs hub isn't fully live yet (no techdomUrl set), but
// clicking the card opens a preview of what's coming instead of doing
// nothing. Once a real techdomUrl is added to SITE_CONFIG, unlockWhenReady()
// swaps the card over to a real link and this preview stops opening (see the
// SITE_CONFIG.techdomUrl check below), so nothing needs to be removed by
// hand later. Note: is-unlocked is also used purely for styling elsewhere
// (e.g. keeping a card at full brightness), so it can't be used here to mean
// "a real link replaced the button" — SITE_CONFIG.techdomUrl is the only
...
function setupTechdomPreview() {
  ...
  function openModal() {
    if (SITE_CONFIG.techdomUrl) return; // real link takes over once live
...
// ---------- Techdom programs tabs ----------
// Scales to any number of programs — add a new .modal__tab button + matching
// .program-card panel in index.html and this wires itself up automatically,
// no changes needed here.
function setupTechdomTabs() {
  const tabs = Array.from(document.querySelectorAll(".modal__tab"));
  ...
  function activateTab(tab) {
    ...
    document.querySelectorAll(".modal__panels .program-card").forEach((panel) => {
...
// ---------- Techdom platform picker ----------
// The popup always opens on the platform picker (Windows/Android/iOS).
// Android and iOS are disabled <button>s for now, so clicks on them do
// nothing automatically — no extra guard code needed. When a platform is
// ready, drop its "disabled" attribute in index.html, add its own
// .modal__platform-view block, and wire its button up the same way
// windowsBtn is wired below.
function setupTechdomPlatforms() {
  const platformsView = document.getElementById("modalPlatforms");
  const windowsView = document.getElementById("modalWindowsView");
  const windowsBtn = document.getElementById("platform-windows");
  const backBtn = document.getElementById("modalBackBtn");
  const card = document.getElementById("techdom-card");
  const trigger = document.getElementById("techdomCardBtn");
  if (!platformsView || !windowsView) return;

  function showPlatforms() {
    windowsView.hidden = true;
    platformsView.hidden = false;
  }

  function showWindows() {
    platformsView.hidden = true;
    windowsView.hidden = false;
  }

  if (windowsBtn) windowsBtn.addEventListener("click", showWindows);
  if (backBtn) backBtn.addEventListener("click", showPlatforms);

  // Always start fresh at the platform picker each time the popup is opened
  if (card) card.addEventListener("click", showPlatforms);
  if (trigger) trigger.addEventListener("click", showPlatforms);
}
